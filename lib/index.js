...
const noIfResOkNoElse = require('./rules/no-if-res-ok-no-else')
const requireEnvelopeCheck = require('./rules/require-envelope-check')
const requireWaiverClassification = require('./rules/require-waiver-classification')
...
const plugin = {
  rules: {
    ...
    'no-if-res-ok-no-else': noIfResOkNoElse,
    'require-envelope-check': requireEnvelopeCheck,
    'require-waiver-classification': requireWaiverClassification,
  },
}
...
plugin.configs = {
  ...
  recommended: {
    ...
    rules: {
      ...
      // Config-driven: inert until the repo registers its envelope-bearing
      // clients via rule options (see rule doc). Kept at error so registered
      // envelopes block the merge like every other rule.
      '@zantha-ltd/fail-fast/require-envelope-check': 'error',

      // require-waiver-classification is DELIBERATELY ABSENT from both
      // recommended configs. It reads existing eslint-disable comments, and
      // every waiver in the estate predates the convention it asks for, so
      // enabling it by default would fail every onboarded repo the day this
      // publishes. Repos opt in when ready to migrate:
